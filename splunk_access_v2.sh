...
Hostname=`cat /etc/zabbix/zabbix_agentd.conf | grep Hostname= | grep -v "#"`
                ...
                #var_top_IPs=`netstat -ntu | awk ' $5 ~ /^[0-9]/ {print $5}' | cut -d: -f1 | sort | uniq -c | sort -nr | head -n10 | awk '{print $2}'| tr  '\n'  ' '`
                var_top_ip=`netstat -antu | awk ' $5 ~ /^[0-9]/ {print $5}' | cut -d: -f1 | sort | uniq -c | sort -nr | head -n1 | awk '{print $2}'`
