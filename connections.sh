...
Total=`ls /opt/logs/nginx/*.access.log | wc -l`
        ...
        do
                ...
                cat $LogFile | grep $TimeRange | awk '{print $3}' >> AccessIPList
#               LogIP=`cat $LogFile | grep $TimeRange | awk '{print $3}' | sort | uniq -c | sort -n | wc -l`
#               TotalLogIP=$((${TotalLogIP}+${LogIP}))
        done
cat AccessIPList | sort | uniq -c | sort -n | wc -l
rm -rf AccessIPList
#echo $TotalLogIP
}
