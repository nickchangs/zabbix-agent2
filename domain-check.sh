...
if [ -e /etc/zabbix/domain-check-expire.txt ];then
    cat /dev/null > /etc/zabbix/domain-check-expire.txt
else
    touch /etc/zabbix/domain-check-expire.txt
fi

if [ -e /etc/zabbix/domain-cname.txt ];then
    cat /dev/null > /etc/zabbix/domain-cname.txt
else
    touch /etc/zabbix/domain-cname.txt
fi

if [ -e /etc/zabbix/domain-list.txt ];then
    cat /dev/null > /etc/zabbix/domain-list.txt
else
    touch /etc/zabbix/domain-list.txt
fi

cat /opt/APP/openresty/nginx/conf/vhost/*.conf | grep server_name | grep -v "#server_name" | sed "s/ /\n/g" | sed '/^\s*$/d' | sed "s/;//g" |sed s/^www\.//g |sed s/^m\.//g |sed s/^cdn\.//g |sed s/^p\.//g |sed s/^pay//g | grep -v server_name | grep -v "_" |  sort -n | uniq > /etc/zabbix/domain-list.txt
 ...
 #       REGISTRAR=`cat ${WHOIS_TMP} | ${AWK} -F: '/Updated by Registrar:/ && $2 != "" { REGISTRAR=substr($2,1,17) } END { print REGISTRAR }'`
  ...
  #  then
    ...
    then
	   ...
	   tday=`echo ${tdomdate} | ${CUT} -d "-" -f 3 | ${CUT} -d "T" -f 1`
           DOMAINDATE=`echo "${tday}-${tmonth}-${tyear}"`
    elif [ "${TLDTYPE}" == "com" -o "${TLDTYPE}" == "net" -o "${TLDTYPE}" == "org"  -o "${TLDTYPE}" == "link" -o "${TLDTYPE}" == "blog" -o "${TLDTYPE}" == "cafe" -o "${TLDTYPE}" == "biz" -o "${TLDTYPE}" == "us" -o "${TLDTYPE}" == "mobi" -o "${TLDTYPE}" == "tv" -o "${TLDTYPE}" == "co" -o "${TLDTYPE}" == "cc" ]; # added on 26-aug-2017 by nixCraft
    then
...
prints()
{
    ...
    then
            ...
            CNAME=`dig -t CNAME $1 +noall +short`
            echo "$Hostname,domain=$1,status=$2,date=$3,days=$4,registrar=$5,cname=$CNAME" | nc 35.194.220.214 -u 514 -w 1
        if [ ${DAY} -lt 90 ]
        then
            echo "$Hostname,Domain=$1,Expire=$3,dyas=$DAY,Cname=$CNAME" >> /etc/zabbix/domain-check-expire.txt
        fi
    fi
}
...
SERVERFILE="/etc/zabbix/domain-list.txt"
